...
public class CostModel {
	...
	double volume_tank;
	public CostModel(double duration, Mass m, double power, Launcher launcher) {
	...
	private static String simplified(double cost, double factor) {
		cost = cost/factor;
		return ((cost > 1e6) ?
				(cost - cost%1e4)/1e6 +" M Euro" : ((cost > 1e3) ?
						(cost - cost%10)/1e3 +" k Euro" : ((cost > 1) ?
								(cost*100 - (cost*100)%1)/100+" Euro" : (cost*1000 - cost*1000%1)+" e-3 Euro")));
	}
